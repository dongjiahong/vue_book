...
<section :class="['read-content', skinColor, { night: nightMode }]">
    ...
    <div :class="['read-join-shelf', { 'read-opt': isShowOpt }]" @click="addToShelf">
    ...
    <div class="read-content-content">
        ...
        <div class="content-list" v-for="(rc, idx) in readContent" :key="idx" v-if="readContent.length > 0">
            <h3>{{ rc.contentTitle }}</h3>
            <p v-for="(content, idx) in rc.contentList" :key="idx"  v-if="content.trim() !== ''" :style="{ fontSize: fontSize + 'px' }">{{ content }}</p>
        </div>
        <!-- $emit是子组件用来触发自定义事件给父组件;对应的父组件监听子组件的事件是v-on -->
...
export default {
  ...
  props: {
    // 下面是验证规则，表示readContent必须为数组
    readContent: Array
  },

  watch: {
    // 当点击目录进入阅读界面时，滚动条回到顶部
  ...
  computed: {
    ...mapState(["curBook", "nightMode", "skinColor", "fontSize"])
  },
