          ...
          <li v-for="book in shelfBookList" :key="book.id" class="shelf-book-item">
              <router-link :to="{ name: 'read' , params: {id: book.id}}">
                  ...
                  <div class="book-info">
                      <h4 class="title">
                          {{book.title}}
                          <!-- time是过滤器（下面有定义）前面的book.updated会作为time的参数传入 -->
...
export default {
  ...
  computed: {
    ...mapState(["shelfBookList"])
  },
  created() {
    let bookIds = Array.from(this.shelfBookList, book => book.id);
    api.getShelfBookUpdate(bookIds).then(data => {
      this.SHELF_BOOK_UPDATE(data);
    });
  },
  methods: {
    ...mapMutations(["SHELF_BOOK_UPDATE", "DEL_FROM_SHELF"]),
